...
class Startline:
    """
    This class is made for the augmentation of real world images with start line images.
    """
    ...
    def draw_startline(start_line_rows, start_line_colums, patch_size):
        ...
        Parameters:
            ...
            patch_size : int
                Amount of pixels that sedcribe the height and width of a single square.
    ...
    def get_birds_eye_view(render_object, camera_image, camera_mask, interpolation):
        ...
        bird_mask = cv2.warpPerspective(zeros_1_large, inv(render_object.h_segmentation), (3000, 3000), flags=interpolation)
    ...
    def get_camera_view(render_object, bird_image, bird_mask, interpolation):
        ...
        Return:
            ...
            camera_mask
        """
        camera_mask_large = cv2.warpPerspective(bird_mask, render_object.h_segmentation, (640, 480), flags=interpolation)
        camera_image_large = cv2.warpPerspective(bird_image, render_object.h_segmentation, (640, 480), flags=interpolation)
        camera_mask = cv2.resize(camera_mask_large, (128, 128), interpolation = interpolation)
        camera_image = cv2.resize(camera_image_large, (128, 128), interpolation = interpolation)
    ...
    def visualize_startline(bird_mask, bird_image, camera_image, camera_mask, startline_image, startline_mask, interpolation):
        ...
        camera_mask = cv2.resize(camera_mask, (1000, 500), interpolation = interpolation)
        camera_image = cv2.resize(camera_image, (1000, 500), interpolation = interpolation)
